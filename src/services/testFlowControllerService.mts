...
import { SetupConfigRaw } from "../models/models.mjs";
...
export class TestFlowControllerService {
  ...
  public async runTests() {
    ...
    try {
      ...
      await this.runTestLogic(testSetupConfigParsed);
    } catch (error) {
  ...
  private async runTestLogic(setupConfig: SetupConfigRaw) {
    ...
    if (dbOptions) {
      if (dbOptions.dbms === "postgresql") {
        dbService = new Postgres();
      } else if (dbOptions.dbms === "mysql") {
        dbService = new Mysql();
      } else {
        console.log(`\n`);
        console.log(`    ------------------------------------`);
        console.log(`    🛑 Ending Test Run: Database "${dbOptions.dbms}" is not supported.`);
        return;
      }
      await dbService.connect(dbOptions);
    }
